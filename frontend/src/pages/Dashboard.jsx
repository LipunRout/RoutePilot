...
const buildAchievements = (roadmaps) => {
  ...
  const totalPhasesDone   = roadmaps.reduce((a, r) => a + (r.completed_phases || 0), 0);
  const xp                = totalRoadmaps * 100;
  const dates             = roadmaps.map((r) => new Date(r.created_at)).sort((a, b) => a - b);
  const daysSinceFirst    = dates.length > 0
    ? Math.floor((new Date() - dates[0]) / (1000 * 60 * 60 * 24))
    : 0;

  return [
    { icon: "⚡", label: "First roadmap",  earned: totalRoadmaps >= 1,     desc: totalRoadmaps >= 1     ? "Created your first roadmap!"  : "Create your first roadmap" },
    { icon: "🔥", label: "7-day streak",   earned: daysSinceFirst >= 7,    desc: daysSinceFirst >= 7    ? "Used RoutePilot for 7+ days!" : `${daysSinceFirst}/7 days` },
    { icon: "✦",  label: "Phase master",   earned: totalPhasesDone >= 5,   desc: totalPhasesDone >= 5   ? "Completed 5+ phases!"         : `${totalPhasesDone}/5 phases done` },
    { icon: "🚀", label: "Roadmap complete",earned: completedRoadmaps >= 1, desc: completedRoadmaps >= 1 ? "Finished a full roadmap!"     : "Complete a full roadmap" },
    { icon: "📚", label: "3 roadmaps",     earned: totalRoadmaps >= 3,     desc: totalRoadmaps >= 3     ? "Created 3 roadmaps!"          : `${totalRoadmaps}/3 roadmaps` },
    { icon: "🎯", label: "30-day learner", earned: daysSinceFirst >= 30,   desc: daysSinceFirst >= 30   ? "Learning for 30+ days!"       : `${daysSinceFirst}/30 days` },
    { icon: "💎", label: "500 XP",         earned: xp >= 500,              desc: xp >= 500              ? "Earned 500+ XP!"              : `${xp}/500 XP` },
    { icon: "🏆", label: "All phases done", earned: totalPhasesDone >= 10, desc: totalPhasesDone >= 10  ? "Mastered 10+ phases!"         : `${totalPhasesDone}/10 phases` },
  ];
};
...
const CAT_COLORS = {
  it:      { color: "#00c97a", glow: "rgba(0,201,122,0.12)",  border: "rgba(0,201,122,0.25)",  icon: "💻" },
  nonit:   { color: "#0ea5e9", glow: "rgba(14,165,233,0.12)", border: "rgba(14,165,233,0.25)", icon: "📊" },
  govt:    { color: "#f59e0b", glow: "rgba(245,158,11,0.12)", border: "rgba(245,158,11,0.25)", icon: "🏛️" },
  biz:     { color: "#a855f7", glow: "rgba(168,85,247,0.12)", border: "rgba(168,85,247,0.25)", icon: "🚀" },
  default: { color: "#00c97a", glow: "rgba(0,201,122,0.12)",  border: "rgba(0,201,122,0.25)",  icon: "🗺️" },
};
...
const EmptyRoadmaps = () => (
  <div style={{ padding: "40px 24px", textAlign: "center", background: "var(--bg-2)", border: "1px dashed var(--border)", borderRadius: 12 }}>
    <div style={{ fontSize: "2.2rem", marginBottom: 10 }}>🗺️</div>
    <div style={{ fontFamily: "'Inter',sans-serif", fontSize: "0.95rem", fontWeight: 600, color: "var(--text-1)", marginBottom: 6, letterSpacing: "-0.02em" }}>No roadmaps yet</div>
    <div style={{ fontFamily: "'Inter',sans-serif", fontSize: "0.82rem", color: "var(--text-3)", marginBottom: 20, lineHeight: 1.6 }}>Generate your first AI-powered career roadmap</div>
    <Link to="/category" style={{ padding: "9px 22px", background: "var(--primary)", color: "#000", borderRadius: 9, fontFamily: "'Inter',sans-serif", fontSize: "0.825rem", fontWeight: 600, textDecoration: "none", display: "inline-block" }}>
      Create roadmap →
    </Link>
  </div>
);
...
export default function Dashboard() {
  ...
  // ← useState calls are NOW INSIDE the component ✅
  const [achievements, setAchievements] = useState([]);
  const [activity,     setActivity]     = useState([
    { day: "Mon", hrs: 0 }, { day: "Tue", hrs: 0 }, { day: "Wed", hrs: 0 },
    { day: "Thu", hrs: 0 }, { day: "Fri", hrs: 0 }, { day: "Sat", hrs: 0 }, { day: "Sun", hrs: 0 },
  ]);
    ...
    getUserRoadmaps()
      .then(({ data }) => {
        const mapped = data.roadmaps.map((rm) => {
          ...
          return {
            id: rm.id, role: rm.role, category: rm.category,
            color: cat.color, glow: cat.glow, border: cat.border, icon: cat.icon,
            ...
            lastActive:      rm.last_active_at
              ? new Date(rm.last_active_at).toLocaleDateString("en-IN", { day: "numeric", month: "short" })
              : "Never",
            timeline: rm.form_data?.timeline ? `${rm.form_data.timeline} months` : "—",
            status:   rm.status || "active",
          };
        });
        setRoadmaps(mapped);
        setAchievements(buildAchievements(data.roadmaps));
        setActivity(buildActivity(data.roadmaps));
      })
      .catch(console.error)
  ...
  const USER = {
    ...
    joinDate: user?.created_at
      ...
      : "—",
    streak: 0, totalHours: 0,
    xp: roadmaps.length * 100,
  };

  /* ── Recent activity from real roadmaps ── */
  const UPDATES = roadmaps.length > 0
    ? roadmaps.slice(0, 5).map((rm) => ({ icon: "✦", text: `Roadmap created: ${rm.role}`, time: rm.lastActive, color: rm.color }))
    : [{ icon: "⚡", text: "No activity yet — create your first roadmap!", time: "now", color: "#00c97a" }];
  ...
  return (
    <>
      <style>{`
        ...
        .db-hero { margin-top: 64px; background: var(--bg-1); border-bottom: 1px solid var(--border); padding: 40px 24px 0; position: relative; overflow: hidden; }
        .db-hero-grid { position: absolute; inset: 0; background-image: linear-gradient(rgba(255,255,255,0.02) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,0.02) 1px, transparent 1px); background-size: 48px 48px; mask-image: radial-gradient(ellipse 100% 100% at 50% 0%, black 20%, transparent 100%); pointer-events: none; }
        .db-hero-orb { position: absolute; width: 500px; height: 300px; border-radius: 50%; background: radial-gradient(circle, rgba(0,201,122,0.06) 0%, transparent 65%); top: -80px; right: -100px; pointer-events: none; }
        .db-hero-inner { max-width: 1100px; margin: 0 auto; position: relative; z-index: 1; }
        .db-welcome { display: flex; align-items: flex-start; justify-content: space-between; gap: 24px; margin-bottom: 28px; flex-wrap: wrap; animation: dbFade 0.5s ease both; }
        .db-avatar-row { display: flex; align-items: center; gap: 14px; }
        .db-avatar { width: 52px; height: 52px; border-radius: 50%; background: linear-gradient(135deg, #00c97a, #0ea5e9); display: flex; align-items: center; justify-content: center; font-family: 'Inter', sans-serif; font-size: 1rem; font-weight: 700; color: #000; flex-shrink: 0; box-shadow: 0 0 0 3px rgba(0,201,122,0.2); }
        ...
        .db-name-inter { font-family: 'Inter', sans-serif; font-size: 1.4rem; font-weight: 700; color: var(--text-1); letter-spacing: -0.05em; line-height: 1.1; }
        .db-name-cormorant { font-family: 'Cormorant Garamond', serif; font-size: 1.6rem; font-weight: 600; font-style: italic; line-height: 1.1; background: linear-gradient(135deg, #00c97a 20%, #0ea5e9 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; }
        .db-welcome-actions { display: flex; gap: 8px; flex-wrap: wrap; align-items: center; }
        .db-btn { display: inline-flex; align-items: center; gap: 7px; padding: 0 16px; height: 38px; border-radius: 9px; font-family: 'Inter', sans-serif; font-size: 0.825rem; font-weight: 500; cursor: pointer; transition: all 0.15s ease; text-decoration: none; white-space: nowrap; }
        ...
        .db-stats-strip { display: grid; grid-template-columns: repeat(4, 1fr); gap: 1px; background: var(--border); border: 1px solid var(--border); border-bottom: none; border-radius: 14px 14px 0 0; overflow: hidden; animation: dbFade 0.5s ease 0.1s both; }
        ...
        .db-stat-num { font-family: 'Cormorant Garamond', serif; font-size: 2rem; font-weight: 600; font-style: italic; line-height: 1; margin-bottom: 4px; background: linear-gradient(135deg, #00c97a, #0ea5e9); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; }
        .db-stat-label { font-family: 'Inter', sans-serif; font-size: 0.75rem; color: var(--text-3); font-weight: 400; }
        .db-tabs { display: flex; border-bottom: 1px solid var(--border); background: var(--bg-1); position: sticky; top: 64px; z-index: 100; animation: dbFade 0.5s ease 0.15s both; }
        .db-tab { padding: 12px 20px; font-family: 'Inter', sans-serif; font-size: 0.85rem; font-weight: 500; color: var(--text-3); background: transparent; border: none; border-bottom: 2px solid transparent; cursor: pointer; transition: all 0.15s ease; white-space: nowrap; }
        ...
        .db-card { background: var(--bg-1); border: 1px solid var(--border); border-radius: 14px; padding: 24px; animation: dbFade 0.5s ease both; }
        .db-card-title { font-family: 'Inter', sans-serif; font-size: 0.72rem; font-weight: 600; letter-spacing: 0.09em; text-transform: uppercase; color: var(--text-3); margin-bottom: 18px; padding-bottom: 12px; border-bottom: 1px solid var(--border); display: flex; align-items: center; justify-content: space-between; }
        .db-card-link { font-size: 0.72rem; font-weight: 500; color: var(--primary); text-decoration: none; text-transform: none; letter-spacing: 0; transition: opacity 0.15s ease; }
        .db-card-link:hover { opacity: 0.75; }
        .db-roadmap-list { display: flex; flex-direction: column; gap: 12px; }
        .db-roadmap-card { background: var(--bg-2); border: 1px solid var(--border); border-radius: 12px; padding: 18px 20px; cursor: pointer; transition: all 0.2s ease; text-decoration: none; display: block; position: relative; overflow: hidden; }
        ...
        @media (max-width: 1024px) { .db-grid { grid-template-columns: 1fr; } .db-col-right { display: grid; grid-template-columns: 1fr 1fr; } }
        @media (max-width: 768px) { .db-hero { padding: 32px 18px 0; } .db-body { padding: 24px 18px 60px; } .db-stats-strip { grid-template-columns: repeat(2,1fr); } .db-col-right { grid-template-columns: 1fr; } .db-roadmap-full-card { grid-template-columns: auto 1fr; } .db-rfc-right { display: none; } .db-welcome { flex-direction: column; align-items: flex-start; } }
        @media (max-width: 480px) { .db-achievements { grid-template-columns: repeat(4,1fr); } .db-quick { grid-template-columns: 1fr 1fr; } }
      `}</style>
          ...
          <div className="db-hero-inner">
              ...
              <div className="db-avatar-row">
                ...
                <div className="db-welcome-text">
                  ...
                  <div className="db-name-line">
                    ...
                    <span className="db-name-cormorant">{USER.name.split(" ")[0]}</span>
                  </div>
                </div>
              </div>
              <div className="db-welcome-actions">
                <Link to="/category" className="db-btn db-btn-primary">+ New roadmap</Link>
                <button className="db-btn db-btn-secondary">✉ Email all PDFs</button>
              </div>
            </div>
            ...
            <div className="db-stats-strip" ref={statsRef}>
              {[
                ...
                { n: earnedCount,         s: "",  l: "Achievements" },
                { n: roadmaps.reduce((s,r)=>s+(r.completedPhases||0),0), s:"", l:"Phases done" },
              ].map((s, i) => (
                <div className="db-stat" key={i}>
                  <div className="db-stat-num">{s.n}{s.s}</div>
            ...
            <div className="db-tabs">
              {[
                ...
                <button key={t.id} className={`db-tab ${activeTab === t.id ? "active" : ""}`} onClick={() => setTab(t.id)}>
                  {t.label}
                </button>
              ))}
            </div>
              ...
              <div className="db-col-left">
                ...
                <div className="db-card" style={{ animationDelay: "0s" }}>
                  <div className="db-card-title">
                    Active Roadmaps
                    <Link to="#" className="db-card-link" onClick={() => setTab("roadmaps")}>View all →</Link>
                  </div>
                  <div className="db-roadmap-list">
                    {loadingRoads ? (
                      ...
                      roadmaps.map((rm, i) => (
                        <Link to={`/roadmap/${rm.id}`} className="db-roadmap-card" key={rm.id} style={{ animationDelay: `${i * 0.06}s` }}>
                          <div className="db-roadmap-card-glow" style={{ background: `radial-gradient(ellipse 60% 60% at 20% 50%, ${rm.glow}, transparent)` }} />
                          <div className="db-rc-top">
                            <div className="db-rc-left">
                              <div className="db-rc-icon" style={{ background: rm.glow, borderColor: rm.border }}>{rm.icon}</div>
                            ...
                            <span className={`db-rc-status ${rm.status}`}>
                              {rm.status === "active" ? "● Active" : rm.status === "paused" ? "⏸ Paused" : "✓ Done"}
                            </span>
                          </div>
                          <div className="db-rc-progress-row">
                            <div className="db-rc-progress-bar">
                              <div className="db-rc-progress-fill" style={{ width: `${rm.progress}%`, background: `linear-gradient(90deg, ${rm.color}, ${rm.color}aa)` }} />
                            </div>
                            <span className="db-rc-progress-pct" style={{ color: rm.color }}>{rm.progress}%</span>
                          </div>
                          <div className="db-rc-meta">
                            <span className="db-rc-phases">{rm.completedPhases}/{rm.totalPhases} phases</span>
                            <span className="db-rc-last">Active {rm.lastActive}</span>
                          </div>
                        </Link>
                      ))
                    )}
                        ...
                        <div className="db-chart-col" key={a.day}>
                          <div className="db-chart-bar-wrap">
                            <div
                              ...
                              title={`${a.hrs} roadmaps`}
                            />
                          </div>
                          <span className="db-chart-day" style={{ fontWeight: isToday ? "bold" : "normal" }}>{a.day}</span>
                        </div>
                      );
                    })}
                  </div>
                ...
                <div className="db-card" style={{ animationDelay: "0.15s" }}>
                  <div className="db-card-title">
                    Achievements
                    <span style={{ fontFamily:"'Inter',sans-serif", fontSize:"0.72rem", color:"var(--primary)", fontWeight:500, textTransform:"none", letterSpacing:0 }}>
                      {earnedCount}/{achievements.length} earned
                    </span>
                  </div>
                  <div className="db-achievements">
                    {achievements.map((a, i) => (
                      <div key={i} className={`db-ach ${!a.earned ? "locked" : ""}`} title={a.desc}>
              ...
              <div className="db-col-right">
                ...
                <div className="db-streak-card">
                  ...
                  <div className="db-streak-dots">
                    {Array.from({ length: 14 }).map((_, i) => (
                      <div key={i} className="db-streak-dot" style={{ background: i < USER.streak % 14 ? "#f59e0b" : "var(--bg-3)" }} />
                    ))}
                  </div>
                  <div className="db-streak-tip">Keep going — you're on a roll</div>
                </div>

                {/* XP */}
                <div className="db-xp-card">
                  <div className="db-card-title" style={{ marginBottom: 14 }}>Experience Points</div>
                  <div className="db-xp-top">
                    <div>
                      <div className="db-xp-val">{cXP.toLocaleString()} XP</div>
                      <div className="db-xp-label">Level {Math.floor(USER.xp / 500) + 1} — Navigator</div>
                    </div>
                    <div className="db-xp-next">Next: {(Math.floor(USER.xp / 500) + 1) * 500} XP</div>
                  </div>
                  <div className="db-xp-bar">
                    <div className="db-xp-fill" style={{ width: `${Math.min((USER.xp / 1000) * 100, 100)}%` }} />
                  </div>
                  <div className="db-xp-levels"><span>0</span><span>250</span><span>500</span><span>750</span><span>1000</span></div>
                </div>
                ...
                <div className="db-card" style={{ animationDelay: "0.2s" }}>
                  ...
                  <div className="db-quick">
                    {[
                      { icon: "⚡", title: "New Roadmap", desc: "Start a path",       to: "/category" },
                      { icon: "◈", title: "Continue",    desc: "Pick up where left", to: roadmaps.length > 0 ? `/roadmap/${roadmaps[0].id}` : "/category" },
                      { icon: "✉", title: "Email PDFs",  desc: "All roadmaps",       to: "#" },
                      { icon: "◎", title: "Explore",     desc: "Browse careers",     to: "/category" },
                    ].map((q) => (
                  ...
                  <div className="db-feed">
                    {UPDATES.map((u, i) => (
                      <div className="db-feed-item" key={i}>
                        <div className="db-feed-icon" style={{ background: u.color+"18", borderColor: u.color+"44", color: u.color }}>{u.icon}</div>
          ...
          {activeTab === "roadmaps" && (
            <div className="db-roadmaps-tab">
              {loadingRoads ? (
                ...
                roadmaps.map((rm, i) => (
                  <Link to={`/roadmap/${rm.id}`} key={rm.id} className="db-roadmap-full-card" style={{ animationDelay: `${i * 0.07}s` }}>
                    <div style={{ position:"absolute", inset:0, borderRadius:14, background:`radial-gradient(ellipse 40% 80% at 0% 50%, ${rm.glow}, transparent)`, pointerEvents:"none" }} />
                    <div className="db-rfc-icon" style={{ background: rm.glow, borderColor: rm.border }}>{rm.icon}</div>
                    <div className="db-rfc-info">
                      <div className="db-rfc-role">{rm.role}</div>
                      <div className="db-rfc-sub">{rm.category} · {rm.timeline}</div>
                      <div className="db-rfc-progress-row">
                        <div className="db-rfc-bar"><div className="db-rfc-fill" style={{ width:`${rm.progress}%`, background:`linear-gradient(90deg, ${rm.color}, ${rm.color}99)` }} /></div>
                        <span className="db-rfc-pct" style={{ color: rm.color }}>{rm.progress}%</span>
                        <span className={`db-rc-status ${rm.status}`}>{rm.status==="active"?"● Active":rm.status==="paused"?"⏸ Paused":"✓ Done"}</span>
                      </div>
                    </div>
                    <div className="db-rfc-right">
                      <div className="db-rfc-actions">
                        <Link to={`/roadmap/${rm.id}`} className="db-rfc-btn db-rfc-btn-primary" onClick={(e)=>e.stopPropagation()}>
                          {rm.status==="completed"?"Review":"Continue →"}
                        </Link>
                        <button className="db-rfc-btn db-rfc-btn-secondary" onClick={(e)=>e.stopPropagation()}>✉ PDF</button>
                      </div>
                      <div style={{ fontFamily:"'Inter',sans-serif", fontSize:"0.72rem", color:"var(--text-3)" }}>
                        {rm.completedPhases}/{rm.totalPhases} phases · Last active {rm.lastActive}
                      </div>
                    </div>
                  </Link>
                ))
              ...
              <Link to="/category" className="db-new-roadmap">
                ...
                <div className="db-new-roadmap-sub">Explore 33+ career paths across IT, Non-IT, Government & Business</div>
              </Link>
            </div>
          )}
              ...
              <div className="db-card" style={{ animationDelay:"0s" }}>
                ...
                <div className="db-chart" style={{ height:120 }}>
                  {activity.map((a, i) => (
                    <div className="db-chart-col" key={a.day}>
                      <div className="db-chart-bar-wrap">
                        <div className="db-chart-bar" style={{ height:`${(a.hrs/maxHrs)*100}%`, background: a.hrs===maxHrs ? "linear-gradient(180deg,#00c97a,#00c97a66)" : "linear-gradient(180deg,var(--border-hover),var(--bg-3))" }} />
                      </div>
                      <span className="db-chart-day">{a.day} · {a.hrs}</span>
                    </div>
                  ))}
                </div>
              </div>
